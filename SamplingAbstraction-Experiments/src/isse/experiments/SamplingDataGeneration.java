...
/**
 * This class serves to create an AVPP purely consisting of physical power plants and perform sampling to obtain data of
 * functional dependencies (rate of change and costs)
 * 
 * @author alexander
 *
 */
public class SamplingDataGeneration implements PowerPlantSource {
	...
	// should power plants be drawn to the available distribution of types or uniformly
	 ...
	 */
	protected final boolean sampleCosts = true;
	protected final boolean samplePdeltaPos = false;
	protected final boolean samplePdeltaNeg = false;
	...
	public void run() {
		...
		PowerPlantUtil.populateDefaultSamplingModel(solver, model, avpp, powerPlants);
		sa.setSolver(solver);
		...
		if (sampleCosts) {
			...
			PowerPlantUtil.writeData("results/" + jobName + "_costs.csv", costFunction);
		}

		if (samplePdeltaPos) {
			PiecewiseLinearFunction pdeltaPos = sa.getPiecewiseLinearFunction(AbstractionParameterLiterals.DEXP_POWER + "Init", false);
			PowerPlantUtil.writeData("results/" + jobName + "_pdelta-pos.csv", pdeltaPos);
		}

		if (samplePdeltaNeg) {
			PiecewiseLinearFunction pdeltaNeg = sa.getPiecewiseLinearFunction(AbstractionParameterLiterals.DEXP_POWER + "Init", true);
			PowerPlantUtil.writeData("results/" + jobName + "_pdelta-neg.csv", pdeltaNeg);
		}

	}

	public List<PowerPlantData> drawPowerPlants() {
		List<PowerPlantData> powerPlants = new ArrayList<PowerPlantData>(numberOfPlants);
	...
	public static void main(String[] args) {
		...
		SamplingDataGeneration generation = new SamplingDataGeneration(generator, "firstTrial");
