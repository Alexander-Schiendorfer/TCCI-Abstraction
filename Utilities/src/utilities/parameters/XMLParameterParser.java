...
public class XMLParameterParser extends DefaultHandler2 {
	...
	public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
		if (qName.equalsIgnoreCase("sweep")) {
			...
			// in case the name/type/value are not available, we return
			if (parameterName == null || parameterType == null || parameterValueString == null) {
				XMLParameterParser.LOG.debug("One of the necessary attributes seems to be not available. Parameter name: '" + parameterName
						+ "', parameter type: '" + parameterType + "', parameter value: '" + parameterValueString
						+ "'. Necessary attributes are 'name', 'constant_type', and 'value'.");
				return;
			}

			// now create the right parameter value object dependent on the specified type
			...
			// Boolean
				...
				if (!parameterValueString.equalsIgnoreCase("true") && !parameterValueString.equalsIgnoreCase("false")) {
					XMLParameterParser.LOG.error("Parameter '" + parameterName + "' has not a valid value (true or false)! Given value is '"
